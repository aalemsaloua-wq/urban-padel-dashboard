...
def _est_entree_ca(code: str, fnr: str, libelle: str) -> bool:
    """Retourne True si cette ligne est une entrée CA espèces."""
    codes_ca = [c.upper() for c in CAISSE_CODES_CA]
    return (
        code.upper() in codes_ca or
        fnr.upper()  in codes_ca or
        libelle.upper().startswith('CA DU') or
        libelle.upper().startswith('CA ') or
        libelle.upper() == 'CA' or
        "CHIFFRE D'AFFAIRE" in libelle.upper() or
        "CHIFFRE D AFFAIRE" in libelle.upper()
    )
